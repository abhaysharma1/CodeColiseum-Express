...
const splitAggregatedInputCases = (input: string, expectedCount: number): string[] => {
  ...
  const targetCount =
    Number.isFinite(declaredCount) && declaredCount > 0 ? declaredCount : expectedCount;
...
const sanitizeSourceCode = (code: string): string => {
  return (
    code
      .replace(/\u00A0/g, " ")
      .replace(/[\u200B-\u200D\uFEFF]/g, "")
...
export async function runRawCodeService(
  ...
  const sanitizedCases: TestCase[] = rawCases
    .map((testCase) => ({
  ...
  const finalCode = sanitizeSourceCode(code);
  ...
  try {
    const result = await axios.post<PistonExecutionResult>(pistonExecuteUrl, payload, {
      headers: {
  ...
  const expandedInputs = shouldExpandAggregatedCase
    ? getInputBlocks(sanitizedCases[0]?.input ?? "", expectedBlocksByCase[0].length)
    : [];

  const normalizedCases: TestCase[] = shouldExpandAggregatedCase
